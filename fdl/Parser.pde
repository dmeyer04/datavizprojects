class Parser {
  ...
  Parser(String filename) {
    ...
    int n = parseInt(lines[0]); // n nodes in graph
    int i = 1;
    String[] spline = split(lines[i],",");
    while (spline.length > 1) {
      int id = parseInt(spline[0]);
      float mass = parseFloat(spline[1]);
      float radius = (float)(mass * MASS_SCALE);
      float x = random(0 + radius, width - radius);
      float y = random(0 + radius, height - radius);
      float z = random(0 + radius, height - radius);
      balls.put(id, new Ball(new PVector(x,y,z), mass, id));
      spline = split(lines[i++],",");
    } 
    for (int j = i; j < lines.length; j++) {
      spline = split(lines[j],",");
      int id1 = parseInt(spline[0]);
      int id2 = parseInt(spline[1]);
